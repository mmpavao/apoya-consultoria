 ...
 *
 * SEGURANÇA: Token MCP lido de Deno.env.get("SERPRO_TOKEN").
 * Configurar via: supabase secrets set SERPRO_TOKEN=<valor>
 * Nunca insira o valor do token diretamente neste arquivo.
 */
...
async function callMcp(toolName: string, args: Record<string, string>): Promise<any> {
  if (!MCP_BEARER) return null; // token não configurado — falha silenciosa
  try {
