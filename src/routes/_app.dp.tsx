...
function DpPage() {
  ...
  // KPIs globais
  const totalAtivos  = useMemo(() => todosFuncionarios.filter(f => f.status === "ativo").length, [todosFuncionarios]);
  ...
  function abrirNovoFuncionario(empresaId: string) {
    setNovoFuncEmpresa(empresaId);
  ...
  return (
    <div className="space-y-6 animate-fade-up p-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label:"Funcionários Ativos",  val: totalAtivos,            icon:<Users className="h-5 w-5"/>,  cls:"text-foreground" },
          { label:"Folhas Abertas",        val: 0,                      icon:<Users className="h-5 w-5"/>,  cls:"text-amber-600" },
          { label:"Férias Vencendo (30d)", val: 0,                      icon:<Users className="h-5 w-5"/>,  cls:"text-amber-600" },
          { label:"Rescisões do Mês",      val: totalDemitidos,         icon:<Users className="h-5 w-5"/>,  cls:"text-red-600" },
        ].map(k => (
          <div key={k.label} className="surface-card px-4 py-3 flex items-center gap-3">
            <div className="p-2 rounded-lg bg-primary/10 text-primary shrink-0">{k.icon}</div>
            <div>
                ...
                <tr key={cliente.id}>
                  ...
                  <td>
                    <div className="flex items-center gap-2">
                      <button
                        onClick={() => abrirNovoFuncionario(cliente.id)}
