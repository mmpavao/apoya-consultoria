      ...
      try {
        ...
        if (data) {
          setCert({
            ...
            focusEntregue:      !!data.focus_cert_enviado,
            focusEntregueEm:    data.focus_cert_enviado_em,
            focusRef:           data.focus_cert_ref,
          });
        }
  ...
  async function handleUpload() {
    ...
    try {
      ...
      let certMeta = {
        ...
        validoAte: "",
        tipo:      "A1" as "A1" | "A3",
        focusRef:  undefined as string | undefined,
      };
      ...
      try {
        const { data: nfResp } = await supabase.functions.invoke("upload-certificate-focus", {
          body: { pfxBase64: b64, password: senha, cnpj: certMeta.cnpj }
        });
        if (nfResp?.ok && nfResp.certId) {
          focusRef = nfResp.certRef;
          focusUploadedAt = new Date().toISOString();
        }
      } catch { /* Focus NF-e não crítico */ }
      ...
      const row: Record<string, unknown> = {
        ...
        focus_cert_ref:      focusRef || undefined,
